<x-tabler-layout title="Edit Employee">
    <x-slot name="header">
        <div class="row align-items-center">
            <div class="col">
                <h2 class="page-title">Edit Employee</h2>
                <div class="page-subtitle">Update employee profile details, salary rates, and assignments</div>
            </div>
            <div class="col-auto">
        ...
        <div class="col-md-8">
            <x-card>
                <form method="POST" action="{{ route('employees.update', $employee) }}">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <x-form.input name="name" label="Employee Name" :value="old('name', $employee->name)" required />
                        </div>
                        <div class="col-md-6">
                            <x-form.select name="role" label="Role" :options="$roles" :selected="old('role', $employee->role)" required />
                        </div>
                    </div>

                    <div class="row mt-3">
                        <div class="col-md-6">
                            <x-form.input name="base_salary" label="Base Monthly Salary (₹)" type="number" step="0.01" :value="old('base_salary', $employee->base_salary)" />
                        </div>
                        <div class="col-md-6">
                            <x-form.input name="daily_rate" label="Daily Rate (₹)" type="number" step="0.01" :value="old('daily_rate', $employee->daily_rate)" />
                        </div>
                    </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="operational_unit_id" class="form-select @error('operational_unit_id') is-invalid @enderror">
                                    ...
                                    @foreach($operationalUnits as $unit)
                                        <option value="{{ $unit->id }}" {{ old('operational_unit_id', $employee->operational_unit_id) == $unit->id ? 'selected' : '' }}>
