<x-tabler-layout>
    <x-slot name="header">
        <x-page-header title="Record Diesel Stock" subtitle="Update daily tank inventory" :breadcrumbs="[
    ...
    <div class="row row-cards">
        <div class="col-md-6">
            <form action="{{ route('diesel-stocks.store') }}" method="POST">
                @csrf
                <x-card>
                    <div class="card-body" x-data="{ 
                        opening: {{ old('opening_liters', $prevClosing) }}, 
                        purchased: {{ old('purchased_liters', 0) }},
                        closing: {{ old('closing_liters', 0) }},
                        ...
                        <div class="row g-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="operational_unit_id"
                                        ...
                                        @foreach($locations as $loc)
                                            <option value="{{ $loc->id }}" {{ old('operational_unit_id') == $loc->id ? 'selected' : '' }}>{{ $loc->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="form-hint">Yesterday's closing balance:
                                        {{ number_format($prevClosing, 2) }} L</div>
                                </div>
                            </div>
